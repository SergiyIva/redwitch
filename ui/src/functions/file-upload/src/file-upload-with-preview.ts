...
export class FileUploadWithPreview {
  ...
  constructor(uploadId: string, options: Options = {}) {
    ...
    this.options.text.selectedCount =
      selectedCount ?? this.options.text.selectedCount;
    ...
    const inputHidden = this.el.querySelector(
      ".custom-file-container .input-hidden"
    );
    const inputVisible = this.el.querySelector(
      ".custom-file-container .input-visible"
    );
    const imagePreview = this.el.querySelector(
    ...
    this.options.images.successPdfImage =
      successPdfImage ?? this.options.images.successPdfImage;
    ...
    this.options.images.backgroundImage =
      backgroundImage ?? this.options.images.backgroundImage;
  ...
  bindClickEvents() {
    ...
    this.imagePreview.addEventListener("click", (e) => {
      ...
      if (
        target.matches(".custom-file-container .image-preview-item-clear-icon")
      ) {
      ...
      if (target.matches(".custom-file-container .image-preview-item")) {
        ...
        const imageClickedEvent = new CustomEvent(
  ...
  addFiles(files: FileList | File[]) {
    ...
    fileArray.forEach((file) => {
      const fileWithUniqueName = new File(
        [file],
        `${
          file.name || "fallback-name"
        }${UNIQUE_ID_IDENTIFIER}${generateUniqueId()}`,
        {
          type: file.type
        }
      );
  ...
  deleteFileAtIndex(index: number) {
    ...
    const imageDeletedEvent = new CustomEvent(
  ...
  refreshPreviewPanel() {
    ...
    imagePreviewItemsArray.forEach((item) =>
